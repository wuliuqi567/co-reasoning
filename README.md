...
```text
[INFO] 监控日志输出: 当前终端；后台运行时建议重定向到 logs/auto_inner_reroute.log
[INFO] 协同推理日志路径: ...
```
...
2. 检测节点或链路是否故障；只要有故障，每轮都会打印故障节点/链路信息。
3. 每轮打印简要业务数量：总业务数、有路径业务数、无路径业务数。
4. 无故障，或者当前故障不影响业务时，冻结当前业务信息作为稳定基线；故障期间新注入业务默认不受当前故障影响，会在下一轮进入稳定基线。
5. 检测到新的故障签名时，优先用已冻结业务信息判断哪些业务原路径受影响。
6. 对受影响业务调用 `inner_rl_reroute.py` 执行一次模拟重路由，并打印原路径、影响原因和重路由结果。
7. 本轮处理完成后不修改业务路径；实际业务路径变化由其他自动重路由程序完成。
8. 下一轮从获取拓扑、链路指标和业务列表重新开始；如果业务已不受影响，会重新冻结当前业务信息。
9. 同一故障签名下，同一业务和同一路径只模拟一次，避免业务接口尚未更新时重复触发。
10. 如果业务记录没有 `path`，无法判断是否受故障影响，会告警并跳过。
...
```text
auto_inner_reroute.py               # 内场自动故障检测与模拟重路由主入口
